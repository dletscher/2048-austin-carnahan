...
class Player(BasePlayer):
	...
	def heuristic(self, state):
		"""Measures how well tiles maintain decreasing patterns"""
		board = state._board
		monotonicity = 0
		...
		for r in range(4):
			for c in range(3):
				if board[4*r+c] >= board[4*r+c+1] and board[4*r+c] > 0:
					monotonicity += 1
				elif board[4*r+c] > 0 and board[4*r+c+1] > 0:
					monotonicity -= 2  # Penalty for non-monotonic
		
		# Check columns for monotonicity
		for c in range(4):
			for r in range(3):
				if board[4*r+c] >= board[4*(r+1)+c] and board[4*r+c] > 0:
					monotonicity += 1
				elif board[4*r+c] > 0 and board[4*(r+1)+c] > 0:
					monotonicity -= 2  # Penalty for non-monotonic
		
		# Weight to make it comparable to typical scores
		return monotonicity * 500
